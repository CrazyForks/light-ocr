...
`.github/workflows/npm-release.yml` 是仅允许从 `main` 手动触发的发布流程：
...
- 在 macOS arm64/x64、Linux x64 glibc、Windows x64 上分别使用 Node.js 22 和 24，从本地 tarballs 执行 `--ignore-scripts` 安装、CJS/ESM 真实 OCR 与 TypeScript compile test。
- 只有以上 gates 全绿后，`npm-release` GitHub environment 才能读取 `NPM_TOKEN`；先发布五个依赖到 `next`，通过 registry facade 安装后再发布主包，最终禁网运行并可提升到 `latest`。
