...
状态：Accepted direction, production integration pending；WebGPU 已选为 Linux x64 通用加速主线；单机 inference-only PoC 已完成，但尚未通过端到端 PG、形成可发布 runtime 或进入 npm
...
下一步是把已完成的 Linux x64 单机 inference-only PoC 收敛为可发布 qualification：先冻结 runtime 产品拓扑与 artifact lock，再生成 detector/recognizer 的算子、shape、placement、质量和端到端报告。正式资格测试使用显式 `provider=webgpu`，避免 Auto 的 CPU 候选掩盖失败。WebGPU 可得到接受、缩减或拒绝结论；拒绝不阻塞 CPU，且会触发专用后端启动 Gate，而不是自动发布某个厂商 provider。
...
| Perf-2 技术候选 | `webgpu`、`cuda`、`openvino`、`migraphx` 等 | `webgpu` 只有未接入产品的本地 inference PoC，其余尚未实现；均未进入当前公开 npm union，只有通过 HC、provider-local PG、D112 和 release-set 审查后才可能加入 |
...
- `SessionExecutionInfo` 已能按 detector/recognizer 报告 requested/actual provider、device、precision、shape policy、model/runtime/cache identity 和 D111 历史 fallback；D112 selection trace 尚未实现。
...
- ONNX Runtime WebGPU Plugin EP 的初始独立插件版本为 `v0.1.0`，要求兼容的 ORT Core `1.24.4+`；这不等于本项目必须采用该拓扑。
- 初始官方 plugin binaries 覆盖 Windows x64/arm64、Linux x64、macOS arm64；没有 macOS x64、Linux arm64 和移动端。本地 PoC 使用另一条 ORT `1.23.0` monolithic WebGPU build，只能证明技术可行性，不能冒充 `v0.1.0` plugin 的 ABI/分发证据。
...
### 5.2 已完成 PoC 与 qualification 配置
...
- `sessionFallback=error` 是唯一有效迁移值；正式 placement/性能报告使用显式 WebGPU，另行测试其创建失败和 D112 Auto 可跳过原因进入 CPU 的 canary。
- PoC 使用 `preferredLayout=NHWC`，后续 qualification 沿用该值；任何 layout change 都需要完整质量和内存验证。
...
当前产品 ORT `1.22.0` 不能直接加载要求 ORT `1.24.4+` 的初始 WebGPU plugin；本地 PoC 的 ORT `1.23.0` monolithic build 也没有可复用的发布 lock。进入产品接线前必须先冻结 Linux runtime product contract：在 monolithic ORT WebGPU build 与兼容 ORT Core + WebGPU plugin 两种拓扑中选择其一，锁定 ORT tag/commit、plugin（若使用）、Dawn/Tint revision、构建参数、artifact 列表、hash、license 和 SBOM。`.cache` PoC 二进制不得成为生产输入。

进入 Perf-3 前，Linux provider-local release 决策必须在以下方案中选定：

1. 接受 platform package 使用不同但各自精确锁定的 ORT runtime flavor；
...
- 冻结 Linux runtime product contract，选择 monolithic 或 ORT Core + plugin 拓扑并提交可复现 artifact lock；不得复用 `.cache` 二进制。
- 对 detector/recognizer 完成严格与允许 partition 两套算子/opset/shape/placement 报告，正式决定 Preview 是否接受 recognizer 的 3 个 CPU 节点，或先修模型/kernel。
- 跑 `generated-hello-123`、`paddleocr-xfund-form` 和动态 width sweep，记录完整 OCR 的 cold/warm、CPU-s、RSS/VRAM、quality 和 cache。
- 生成压缩/解包包增量、依赖树、license、SBOM 与兼容失败报告。

退出条件：得到可复现的接受、缩减或拒绝技术结论；不修改当前 CPU release package，不进入公开 provider union。

### Phase B — WebGPU Linux Preview Gate

- 按 HC/D111 固定至少两个、目标为三个 GPU vendor 的设备与 driver。
- 实现 provider registry、固定路径 plugin load、D112 `webgpu → cpu` 创建选择、attempt trace 和 device-lost/close 语义。
...
1. Linux WebGPU runtime 采用已验证但待产品化的 monolithic ORT build，还是 ORT `1.24.4+` Core + 独立 plugin；精确 tag/commit、Dawn/Tint revision 和兼容规则是什么？
