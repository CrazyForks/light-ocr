...
- queue depth 0 的 Node 单请求 `absolutePeak <= 同平台同 fixture Core peak + 64 MiB`，并且绝对不超过 `1088 MiB`；
...
- Core tiled warm median 不超过同机、同 fixture、同 pixels/batch/threads 的 Python `tiled_v1` oracle `1.10x`；
- Core tiled warm p95 不超过对应 Python oracle `1.15x`；inference-only median 必须保存为 observation，但不作为跨发行包阻断条件，因为 Core 使用 NuGet/平台原生 ONNX Runtime，Python oracle 使用 PyPI wheel，这个比率同时包含两份官方二进制的构建差异；
- Node tiled median 不超过同机同 fixture Core median `1.10x`，Node p95 不超过 Core p95 `1.15x`；
...
首次实现没有旧 tiled baseline 时，candidate 仍必须通过 absolute memory、relative-to-oracle/Core latency 和全部 quality gates。CI 生成待审 JSON；reviewer 核对 runner identity、result hash、波动、profiler 证据和四平台完整性后，把 baseline 文件作为同一 release PR 的受审变更提交。
